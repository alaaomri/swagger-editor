          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200 flex flex-col">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div className="flex justify-between items-center">
                ...
                {modifiedYaml && (
                  <button
                    disabled={validationResult?.messages?.length > 0 || validationResult?.schemaValidationMessages?.length > 0 || previewLoading}
                    onClick={downloadYaml}
                    className={`flex items-center px-4 py-2 text-white text-sm font-medium rounded-lg transition-colors ${validationResult?.messages?.length > 0 || validationResult?.schemaValidationMessages?.length > 0 || previewLoading ? 'bg-indigo-200 hover:bg-indigo-200' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                  >
